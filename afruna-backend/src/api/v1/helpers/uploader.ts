...
class Multer {
  ...
  private spacesUrl = Config.SPACES_URL;
  private spacesBucket = Config.SPACES_BUCKET_NAME;
  private spacesRegion = Config.SPACES_REGION;
  private spacesAccessKeyId = Config.SPACES_ACCESS_KEY_ID;
  private spacesSecretAccessKey = Config.SPACES_SECRET_ACCESS_KEY;
  private s3;
  private storage;
  ...
  constructor() {
    if (this.useS3 || this.useDigitalOceanSpaces) {
      this.storageType = 's3';
    ...
    this.storage =
      this.useS3 || this.useDigitalOceanSpaces
        ? multerS3({
            s3: this.s3!,
            bucket: this.useS3 ? this.bucket : this.spacesBucket,
            ...
            key: function (req, file, cb) {
              cb(null, Date.now().toString() + '-' + file.originalname.replace(/\s+/g, '-').toLowerCase());
            },
          })
        : this.useDiskStorage
        ? multer.diskStorage({
  ...
  async addObject(body: string, contentType: string) {
    ...
    const params: clientS3.PutObjectCommandInput = {
      Key: key,
      Bucket: this.bucket,
